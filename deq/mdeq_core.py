'''
https://github.com/locuslab/deq/blob/master/MDEQ-Vision/lib/models/mdeq_core.py
'''
...
from deq.utils import get_world_size, get_rank
from deq.core.optimizations import VariationalHidDropout2d, weight_norm
from deq.core.solvers import anderson, broyden
from deq.core.jacobian import jac_loss_estimate, power_method
from deq.layer_utils import list2vec, vec2list, norm_diff, conv3x3, conv5x5
...
class MDEQModule(nn.Module):
  ...
  def _make_branches(self, num_branches, block, num_blocks, num_channels, big_kernels, dropout=0.0):
    ...
    """
    branch_layers = [self._make_one_branch(i, block, num_blocks, num_channels, big_kernels, dropout=dropout) for i in
                     range(num_branches)]
...
class MDEQNet(nn.Module):
  ...
  def forward(self, x, train_step=-1, **kwargs):
    raise NotImplemented  # To be inherited & implemented by MDEQClsNet and MDEQSegNet (see deq.py)